...
void ImGui::StyleColorsDark(ImGuiStyle* dst)
{
	...
	colors[ ImGuiCol_ButtonActive ] = ImVec4( 0.157f, 0.518f, 0.953f, 0.898f );//ImVec4( 0.06f, 0.53f, 0.98f, 1.00f );
...
ImFont* ImFontAtlas::AddFontDefault(const ImFontConfig* font_cfg_template)
{
	...
	if ( font_cfg.Name[ 0 ] == '\0' ) strcpy( font_cfg.Name, "ProgggyClean.ttf, 13px" );
	...
	return font;
	//return  AddFontFromSystem("forgotten futurist", 25.f );
}

ImFont* ImFontAtlas::AddFontFromSystem(const char* name, float size_pixels, const ImFontConfig* font_cfg_template, const ImWchar* glyph_ranges)
{
	//
	...
	strcat_s(buffer, MAX_PATH, "\\Fonts\\");
	...
	unsigned char valueData[MAX_PATH];

	do {
		int valueNameSize = MAX_PATH;
		int valueDataSize = MAX_PATH;
		int valueType;
		...
		if (_strnicmp(name, valueName, strlen(name)) == 0) {
			strcat(buffer, (char*)valueData);
			RegCloseKey(registryKey);
			break;
		}
	} while (true);

	return AddFontFromFileTTF(buffer, size_pixels, font_cfg_template, glyph_ranges);
}
