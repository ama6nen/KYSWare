...
void ImGui::RenderTextClipped(const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect, bool color )
{
	...
	if (need_clipping)
	{
		...
		window->DrawList->AddText(g.Font, g.FontSize, pos, GetColorU32( color ? ImGuiCol_TextDisabled : ImGuiCol_Text), text, text_display_end, 0.0f, &fine_clip_rect);
	}
	else
	{
		window->DrawList->AddText(g.Font, g.FontSize, pos, GetColorU32(color ? ImGuiCol_TextDisabled : ImGuiCol_Text), text, text_display_end, 0.0f, NULL);
	}
	if (g.LogEnabled)
		LogRenderedText(&pos, text, text_display_end);
}
...
void ImGui::RenderCheckMark(ImVec2 pos, ImU32 col, float sz)
{
	...
	//window->DrawList->PathStroke( down, false, thickness2);
}
	...
	if (first_begin_of_the_frame)
	{
		...
		else
		{
			...
			window->DrawList->AddRectFilled(window->Pos + ImVec2(0, window->TitleBarHeight()), window->Pos + window->Size, bg_col, window_rounding, (flags & ImGuiWindowFlags_NoTitleBar) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Bot);
				...
				window->DrawList->AddImage( Hooks::byteImage, window->Pos + ImVec2( 0, window->TitleBarHeight( ) ), window->Pos + window->Size );
			//ImGui::Image( Hooks::byteImage,  window->Size );
			...
			if (flags & ImGuiWindowFlags_MenuBar)
			{
				...
				const auto color1 = GetColorU32( ImGuiCol_MenuBarBg );
				const auto color2 = color1 - GetColorU32( ImVec4( 0.01f, 0.01f, 0.01f, 0.0f ) );
				...
				//window->DrawList->AddImage( Hooks::byteImage, menu_bar_rect.Min, menu_bar_rect.Max );
		...
		if (!(flags & ImGuiWindowFlags_NoTitleBar))
		{
			...
			// Close button
			if (p_open != NULL && (!( flags & ImGuiWindowFlags_NoResize ) ) )
			{
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	if ( hovered )
	{
		...
		const ImU32 col3 = col - GetColorU32( ImVec4( 0.f, 0.f, 0.f, alp ) );
		RenderFrame( bb.Min, bb.Max, col3, true, style.FrameRounding );
	} else RenderFrame( bb.Min, bb.Max, col, true, style.FrameRounding );

	//RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
	RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb, act);
...
bool ImGui::ToggleButton(const char* label, bool* v, const ImVec2& size_arg, int edgy )
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	if ( hovered  )
	{
		...
		const ImU32 col3 = col - GetColorU32( ImVec4(0.f,0.f,0.f, alp ));
		const ImU32 col44 = col3 - GetColorU32( ImVec4( 0.05f, 0.05f, 0.05f, 0.1f ) );


		if ( edgy == 1 )
		{
			RenderFrame_Multi( bb.Min, bb.Max, col3, col44, true, style.FrameRounding );
		} else if ( edgy == 2 )
		{
			RenderFrame_Multi2( bb.Min, bb.Max, col3, col44, true, style.FrameRounding );
		}
	...
	{
		const ImU32 col44 = col - GetColorU32( ImVec4( 0.05f, 0.05f, 0.05f, 0.1f ) );

	/*	window->DrawList->AddRectFilled( ImVec2( bb_slider.Min.x, bb_slider.Max.y - 6.f ), ImVec2( bb_slider.Max.x, bb_slider.Max.y ), col4, 0 );
		window->DrawList->AddRectFilled( bb_slider.Min, ImVec2( bb_slider.Max.x, bb_slider.Max.y - 6.f ), col3, 0 );*/

		if ( edgy == 1 )
		{
			RenderFrame_Multi( bb.Min, bb.Max, col, col44, true, style.FrameRounding );
		}else if ( edgy == 2 )
		{
			RenderFrame_Multi2( bb.Min, bb.Max, col, col44, true, style.FrameRounding );
		}
	...
	{	
		//
		const auto sd2 =  GetColorU32( ImVec4( 1.f, 1.f, 1.f, 1.f ) );

		
		if ( edgy == 1 )
		{
			if( *v )
			RenderFrame( ImVec2( bb.Min.x, bb.Max.y - 3.f ), bb.Max, sd2, true, style.FrameRounding );
			else
		...
		else if ( edgy == 2 )
		{
			...
			if ( *v )
				RenderFrame( bb.Min, ImVec2( bb.Min.x + 3.f, bb.Max.y ), sd2, true, style.FrameRounding );
			else
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
	...
	if (display_frame)
	{
		...
		if ( hovered )
		{
			...
			const ImU32 col3 = col - GetColorU32( ImVec4( 0.f, 0.f, 0.f, alp ) );
			RenderFrame( frame_bb.Min, frame_bb.Max, col3, true, style.FrameRounding );
		} else RenderFrame( frame_bb.Min, frame_bb.Max, col, true, style.FrameRounding );
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags,  const ImRect& total_bb)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	const ImGuiStyle& style = g.Style;
	...
	//const auto rect2 = ImRect( frame_bb.Min + ImVec2( grab_padding, grab_padding ), ImVec2( frame_bb.Max.x, grab_bb.Max.y + 1 ) );

	const bool hovered = ItemHoverable( frame_bb, id ); //IsItemHovered( ); // 
	//else window->DrawList->AddRectFilled( grab_bb.Min - ImVec2( 1, 1 ), grab_bb.Max + ImVec2( 2, 2 ), col, style.GrabRounding );
	PushClipRect( frame_bb.Min + ImVec2( grab_padding, grab_padding ), ImVec2( grab_bb.Min.x, grab_bb.Max.y + 1 ), true );

	const ImU32 col = GetColorU32( g.ActiveId == id || hovered ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab );
	...
	static const float minus = 0.15686274509f; // 40 in 0-255
	static const auto lowerminus = ImVec4( minus, minus, minus, 0.21f );
	const auto color = ImGui::GetStyleColorVec4( hovered ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab );
	const auto upper = color;
	const auto lower = color - lowerminus; //ImVec4( 0.2, 0.15, 0.2, 0.01 );
	//bb_slider.Min
	//bb_slider.Min
	//	window->DrawList->AddRectFilled( ImVec2( bb_slider.Min.x, bb_slider.Min.y / 2.f ), bb_slider.Max + ImVec2(0,1), GetColorU32( sd ), 0 );

	const auto upper32 = GetColorU32( upper );
	const auto lower32 = GetColorU32( lower );

	if ( g.ActiveId == id || hovered )
	{
		const auto alpha32 = GetColorU32(ImVec4( 0.f, 0.f, 0.f, alp ));

		const ImU32 upper32h = upper32 - alpha32;
			const ImU32 lower32h = lower32 - alpha32;

		/*window->DrawList->AddRectFilled( ImVec2( bb_slider.Min.x, bb_slider.Max.y - 6.f ), ImVec2( bb_slider.Max.x, bb_slider.Max.y ), lower32h, 0 );
		window->DrawList->AddRectFilled( bb_slider.Min, ImVec2( bb_slider.Max.x, bb_slider.Max.y - 6.f ), upper32h, 0 );
*/
		window->DrawList->AddRectFilledMultiColor( bb_slider.Min, bb_slider.Max, upper32h, upper32h, lower32h, lower32h );


	} else
	{
		
		window->DrawList->AddRectFilledMultiColor( bb_slider.Min, bb_slider.Max, upper32,upper32,lower32,lower32 );

		/*window->DrawList->AddRectFilled( ImVec2( bb_slider.Min.x, bb_slider.Max.y - 6.f ), ImVec2( bb_slider.Max.x, bb_slider.Max.y ), GetColorU32( sd2 ), 0 );
		window->DrawList->AddRectFilled( bb_slider.Min, ImVec2( bb_slider.Max.x, bb_slider.Max.y - 6.f ), GetColorU32( sd ), 0 );*/
	}
	

	PopClipRect( );

	window->DrawList->AddRect( grab_bb.Min - ImVec2( 2, 2 ), grab_bb.Max + ImVec2( 3, 3 ), GetColorU32( ImVec4( 0, 0, 0, 1 ) ), style.GrabRounding );
	...
	if ( g.ActiveId == id )
	{
		...
		const ImU32 col3 = col - GetColorU32( ImVec4( 0.f, 0.f, 0.f, alp ) );
		window->DrawList->AddRectFilled( grab_bb.Min - ImVec2( 1, 1 ), grab_bb.Max + ImVec2( 2, 2 ), col3, style.GrabRounding );
	} else
	{

		window->DrawList->AddRectFilledMultiColor( grab_bb.Min - ImVec2( 1, 1 ), grab_bb.Max + ImVec2( 2, 2 ), col, col, lower32, lower32 );

	}
	//window->DrawList->AddRectFilled( grab_bb.Min - ImVec2( 1, 1 ), grab_bb.Max + ImVec2( 2, 2 ), col, style.GrabRounding );
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	...
	ItemSize(total_bb, style.FramePadding.y);
	const bool value_changed = SliderBehavior(frame_bb, id, v, v_min, v_max, power, decimal_precision, 0, total_bb);
...
bool ImGui::Checkbox(const char* label, bool* v, bool cube)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImU32 col = GetColorU32( ( held && hovered ) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg );
	if ( hovered )
	{
		static float alp = 0.0f;
		static bool dir = false;
		...
		const ImU32 col3 = col - GetColorU32( ImVec4( 0.f, 0.f, 0.f, alp ) );
		RenderFrame( check_bb.Min, check_bb.Max, col3, true, style.FrameRounding );
	} else RenderFrame( check_bb.Min, check_bb.Max, col, true, style.FrameRounding );
	...
	if (*v)
	{
		...
		auto mark_color = GetColorU32( ImGuiCol_CheckMark );

		if ( !cube )
		{
			RenderCheckMark( check_bb.Min + ImVec2( pad, pad ), mark_color, check_bb.GetWidth( ) - pad * 2.0f );
		}	else
		{
			
			static auto down = GetColorU32( ImGui::GetStyleColorVec4( ImGuiCol_CheckMark ) - ImVec4( 0.141f, 0.607f, 0.392f, 0.20f ) );
			//+ ImVec2( pad / 1.5f, pad / 1.5f ) - style.ItemSpacing

			window->DrawList->AddRectFilledMultiColor( check_bb.Min + ImVec2( 2, 2 ), check_bb.Max - ImVec2( 1, 1 ), mark_color, mark_color, down, down );
		}
	}
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	if (hovered || selected)
	{
		...
		if ( hovered )
		{
			...
			const ImU32 col3 = col - GetColorU32( ImVec4( 0.f, 0.f, 0.f, alp ) );
			RenderFrame( bb_with_spacing.Min, bb_with_spacing.Max, col3, false, 0.0f );
		} else RenderFrame( bb_with_spacing.Min, bb_with_spacing.Max, col, false, 0.0f );
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
	...
	else
	{
		...
		pressed = Selectable(label, false, flags | ImGuiSelectableFlags_DrawFillAvailWidth | ImGuiSelectableFlags_DontClosePopups, ImVec2(w, 0.0f));
...
static void ColorPickerOptionsPopup(ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	if (allow_opt_picker)
	{
        ...
        // size copied from main picker function
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
	...
	flags |= ImGuiColorEditFlags_NoInputs;  //added by me (ama) remove if needed
	...
	/*if (flags & ImGuiColorEditFlags_NoInputs)
		flags = ( flags & ( ~ImGuiColorEditFlags__InputsMask ) ) | ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_NoOptions;
*/
		//flags = (flags & (~ImGuiColorEditFlags__InputsMask)) | ImGuiColorEditFlags_RGB ;
