...
{
public:
    virtual void                SetCacheNotify(IMDLCacheNotify *pNotify) = 0; //9
	...
	virtual vcollide_t*			GetVcollideScaled( MDLHandle_t handle, float flScale ) = 0; //16
    virtual vcollide_t*         GetVCollide(MDLHandle_t handle) = 0;
    virtual unsigned char*      GetAnimBlock(MDLHandle_t handle, int nBlock) = 0;
    virtual virtualmodel_t*     GetVirtualModel(MDLHandle_t handle) = 0;
    virtual int                 GetAutoplayList(MDLHandle_t handle, unsigned short **pOut) = 0; //20
	virtual void				func21( ) = 0; //21
    virtual vertexFileHeader_t* GetVertexData(MDLHandle_t handle) = 0; //22
    virtual void                TouchAllData(MDLHandle_t handle) = 0;
    virtual void                SetUserData(MDLHandle_t handle, void* pData) = 0;
    virtual void*               GetUserData(MDLHandle_t handle) = 0; //25
	virtual void			    func26( ) = 0; //note to self -> this is 23-26 , idk which one exactly
    ...
    virtual const char*         GetModelName(MDLHandle_t handle) = 0; //30
	virtual void				func31( ) = 0;
    virtual virtualmodel_t*     GetVirtualModelFast(const studiohdr_t *pStudioHdr, MDLHandle_t handle) = 0; //32
    virtual void                BeginLock() = 0;  //33
    virtual void                EndLock() = 0; //34
    virtual int*                GetFrameUnlockCounterPtrOLD() = 0; //35 
