...
void ImGui::Render()
{
	...
	if (g.Style.Alpha > 0.0f)
	{
		...
		if (g.IO.MouseDrawCursor && g.IO.Fonts->GetMouseCursorTexData(g.MouseCursor, &offset, &size, &uv[0], &uv[2]))
		{
			...
			g.OverlayDrawList.AddImage( tex_id, pos, pos + size, uv[ 2 ], uv[ 3 ], IM_COL32( 8, 156, 235, 255 ) );       // Black border
			g.OverlayDrawList.AddImage( tex_id, pos, pos + size, uv[ 0 ], uv[ 1 ], IM_COL32( 0, 0, 0, 255 ) ); // White fill
		...
		//From old Kysware with old imgui
...
//	bool allow_opt_alpha_bar = !(flags & ImGuiColorEditFlags_NoAlpha) && !(flags & ImGuiColorEditFlags_AlphaBar);
	if ((!allow_opt_picker) || !ImGui::BeginPopup("context"))
		return;
	ImGuiContext& g = *GImGui;
	if (allow_opt_picker)
	{
		ImVec2 picker_size(g.FontSize * 9, ImMax(g.FontSize * 8 - (ImGui::GetFrameHeight() + g.Style.ItemInnerSpacing.x), 1.0f)); // FIXME: Picker size copied from main picker function
